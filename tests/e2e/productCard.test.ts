...
test.describe('ProductCard component', () => {
  test('displays on sale badge when product is on sale', async ({page}) => {
    ...
    await page.evaluate(() => {
      ...
      if (firstProductCard) {
        ...
        if (container) {
          ...
          badge.className =
            'absolute top-4 left-4 z-10 px-2 py-1 border border-red-500 rounded-[25px] text-sm font-medium text-red-500';
    ...
    const badge = page
      .locator('[data-test-on-sale="true"]')
      .getByText('On Sale!')
      .first();
  ...
  test('does not display badge when product is not on sale', async ({page}) => {
    ...
    await page.evaluate(() => {
      const testBadges = document.querySelectorAll(
        '.product-card div:not(.component-badge)',
      );
    ...
    const regularProducts = page.locator(
      '.product-card:not([data-test-on-sale="true"])',
    );
  ...
  test('color swatches are displayed and can be selected', async ({page}) => {
    ...
    if (swatchCount > 1) {
      const selectedSwatch = productCard
        .locator('.flex.gap-2 button:has(.ring-1)')
        .first();
      const initialAriaLabel =
        (await selectedSwatch.getAttribute('aria-label')) || '';
      ...
      const newSelectedSwatch = productCard
        .locator('.flex.gap-2 button:has(.ring-1)')
        .first();
      const newAriaLabel =
        (await newSelectedSwatch.getAttribute('aria-label')) || '';
  ...
  test('clicking on color swatches updates the displayed product image', async ({
    page,
  }) => {
    await page.goto('/collections/all');
    await page.waitForLoadState('networkidle');
  ...
  test('HoverableImage handles hover interaction correctly', async ({page}) => {
    test.info().annotations.push({
      type: 'issue',
      description:
        'This test might be flaky in CI environments due to headless browser differences',
    });
    await page.goto('/collections/all');
    await page.waitForLoadState('networkidle');
    ...
    const hoverableImage = productCard
      .locator('.recommended-product img')
      .first();
    ...
    const hoverImgContainer = productCard.locator(
      '[data-test-hover-container="true"]',
    );
